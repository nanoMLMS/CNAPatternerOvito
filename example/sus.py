...
pipeline = import_file("./surface.xyz")
...
# The 'CNA Indices' bond property is a two-dimensional array
...
def row_histogram(a):
    ...
    unique, indices, inverse = numpy.unique(
        ca, return_index=True, return_inverse=True
    )
    counts = numpy.bincount(numpy.reshape(inverse, (inverse.shape[0],)))
